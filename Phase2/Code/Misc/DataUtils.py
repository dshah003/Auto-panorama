...
def SetupAll(BasePath, CheckPointPath):
    ...
    LabelsPathTrain = BasePath+'/all_H4pt.p'
...
def ReadLabels(LabelsPathTrain):
    if(not (os.path.isfile(LabelsPathTrain))):
        ...
        TrainLabels = pickle.load( open( LabelsPathTrain, "rb" ) )
...
def SetupDirNames(BasePath): 
    ...
    """
    DirNamesTrain = ReadDirNames(BasePath + '/all_stacked_images.p')        
...
def ReadDirNames(ReadPath):
    ...
    """
    # Read text files
    DirNames = pickle.load( open( ReadPath, "rb" ) )
